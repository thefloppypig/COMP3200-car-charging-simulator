...
namespace CodingConnected.TraCI.NET.Helpers
{
	internal static class TraCICommandHelper
	{
        ...
        internal static TraCIResponse<T> ExecuteGetCommand<T>(TraCIClient client, string id, byte commandType, byte messageType, string param)
        {
            var command = GetCommand(id, commandType, messageType, param);
            var response = client.SendMessage(command);
        ...
        /*internal static TraCIResponse<T> ExecuteGetCommand<T>(TraCIClient client, string id, byte commandType, byte messageType, string format, string values)
        {
            var command = GetCommand(id, commandType, messageType, format, values);
            var response = client.SendMessage(command);
