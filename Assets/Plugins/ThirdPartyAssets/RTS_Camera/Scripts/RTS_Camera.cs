...
namespace RTS_Cam
{
    ...
    {
        private const float rotMax = 90f;
        private const float movMax = 50f;
        ...
        private void Start()
        {
            zoomPos = 1- (transform.position.y/2 - maxHeight)/(minHeight - maxHeight);
        ...
        private void Move()
        {
            if (useKeyboardInput)
            {
                ...
                desiredMove *= (1.01f-zoomPos)*keyboardMovementSpeed;
            ...
            if(usePanning && Input.GetKey(panningKey) && MouseAxis != Vector2.zero)
            {
                ...
                desiredMove = Quaternion.Euler(new Vector3(0f, transform.eulerAngles.y, 0f)) * desiredMove;
                desiredMove = m_Transform.InverseTransformDirection(desiredMove);
                if (desiredMove.sqrMagnitude > movMax * movMax) desiredMove = desiredMove.normalized * movMax;
        ...
        private void Rotation()
        {
            if (!EventSystem.current.IsPointerOverGameObject())
            {
                ...
                {
                    rotY += mouseRotationSpeed * Time.deltaTime * MouseAxis.y;
                    rotX -= mouseRotationSpeed * Time.deltaTime * MouseAxis.x;
                    rotY = Mathf.Clamp(rotY, -rotMax, rotMax);

                    transform.localRotation = Quaternion.Euler(rotY, rotX, 0f);
                    //m_Transform.Rotate(Vector3.up, -MouseAxis.x * Time.deltaTime * mouseRotationSpeed, Space.World);
                    // m_Transform.Rotate(transform.right, MouseAxis.y * Time.deltaTime * mouseRotationSpeed, Space.World);
                }
            }
        }
