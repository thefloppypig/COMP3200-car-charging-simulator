...
namespace PathCreation {
    public class PathCreator : MonoBehaviour {
        ...
        {
            Vector3 worldCentre = bezierPath.CalculateBoundsWithTransform(transform).center;
            ...
            if (worldCentre != transform.position)
            {
                //Undo.RecordObject (creator, "Centralize Transform");
                if (worldCentreToTransform != Vector3.zero)
                {
                    Vector3 localCentreToTransform = MathUtility.InverseTransformVector(worldCentreToTransform, transform, bezierPath.Space);
